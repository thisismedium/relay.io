          ...
          Uuid) 
{
  ...
  function Hub () {
    ...
    this.handle = function (stream) {
      ...
      .on("PutApplication", function (mesg, resp) {
        appDB.putApplicationData(mesg.body, function () {
          resp.reply(Api.Okay());
        });
      })
      .on("DeleteApplicaion", function (mesg, resp) {
        appdDB.deleteApplicationData(mesg.to, function ()  {
          resp.reply(Api.Okay());
        })
      })
